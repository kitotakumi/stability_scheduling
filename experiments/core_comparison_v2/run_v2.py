#!/usr/bin/env python3
"""
core_comparison_v2: コア比較実験 v2 実行スクリプト
...
=== 使い方 ===
  ...
  python run_v2.py --n-trials 10 --n-jobs 4

  # 重みを分割して同じ output-dir に統合
  python run_v2.py --weights "1.0,0 0.9,0.1 0.8,0.2 0.7,0.3 0.6,0.4" \\
      --output-dir results/main --n-jobs 4
  python run_v2.py --weights "0.5,0.5 0.4,0.6 0.3,0.7 0.2,0.8 0.1,0.9 0.0,1.0" \\
      --output-dir results/main --n-jobs 4

  # 本番（30試行）
  python run_v2.py --n-trials 30 --output-dir results/main --n-jobs 4
...
DEFAULT_N_TRIALS = 10
DEFAULT_N_JOBS = 8
REPAIR_TRIGGER_DEFAULT = 50
REPAIR_STRENGTH_DEFAULT = 1
RELINK_TRIGGER_DEFAULT = 50
    ...
    if kind == 'ga':
        ...
        for h in history:
            ...
            if kick_ms is not None and kick_st is not None and np.isfinite(kick_ms) and np.isfinite(kick_st):
                pts.append([float(kick_ms), float(kick_st)])
    return pts
...
def _run_one_task(task):
    ...
    try:
        ...
        uea_points = _extract_uea_points(history, kind)
...
def main():
    parser = argparse.ArgumentParser(
        description='core_comparison_v2: 4手法×11重み コア比較実験')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f'path relink 発動閾値 (デフォルト: {RELINK_TRIGGER_DEFAULT})')
    parser.add_argument(
        '--memetic-ngen', type=int, default=MEMETIC_NGEN,
        help=f'Memetic GA 世代数 (デフォルト: {MEMETIC_NGEN})')
    parser.add_argument(
        '--output-dir', type=str, default=None,
        help='出力先 (デフォルト: results/core_v2_<timestamp>). '
    ...
    if args.output_dir:
        ...
        out_dir = os.path.join(_HERE, 'results', f'core_v2_{ts}')
    ...
    if not pending:
        print("全タスク完了済み。分析は analyze_v2.py を実行してください。")
        return
    ...
    print(f"結果: {out_dir}")
    print(f"分析: python analyze_v2.py --input-dir {out_dir}")
