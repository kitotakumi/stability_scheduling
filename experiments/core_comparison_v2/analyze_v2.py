#!/usr/bin/env python3
"""
core_comparison_v2: コア比較実験 v2 分析スクリプト

run_v2.py が生成した raw データから evaluation_design.md §6 の指標・図表を生成する。
...
=== 使い方 ===
  python analyze_v2.py --input-dir results/core_v2_<timestamp>/
  python analyze_v2.py --input-dir results/main/ --problems la36 la40
  python -u experiments/core_comparison_v2/analyze_v2.py --input-dir experiments/core_comparison_v2/results/pilot_v2b --repr-weights w08_02 --n-jobs 4
...
def _pareto_until_t(hist_list, pts_list, kind, t, baseline=None):
    ...
    for hist, pts in zip(hist_list, pts_list):
        ...
        if idx <= 0:
            continue
        if kind == 'ga':
            ev = hist[idx - 1].get('evaluations')
            n_take = int(ev) if ev else int(idx * len(pts) / len(hist))
        else:
            n_take = min(idx, len(pts))
        n_take = max(0, min(n_take, len(pts)))
        if n_take > 0:
            collected.append(np.array(pts[:n_take]))
    if not collected:
        return np.zeros((0, 2))
...
def _last_pf_update_time(hist, pts, kind, baseline=None):
    ...
    # 各点に cpu_time を割り当て (numpy で一括処理)
    if kind == 'ga':
        ev_vals = np.array([
            int(e['evaluations']) if e.get('evaluations') else 0
            for e in hist
        ])
        cpu_vals = np.array([
            float(e['cpu_time']) if e.get('cpu_time') is not None else 0.0
            for e in hist
        ])
        valid = ev_vals > 0
        if not np.any(valid):
            return float(hist[-1]['cpu_time']) if hist else None
        ev_vals_v = ev_vals[valid]
        cpu_vals_v = cpu_vals[valid]
        order = np.argsort(ev_vals_v, kind='stable')
        ev_sorted = ev_vals_v[order]
        cpu_sorted = cpu_vals_v[order]
        # 各点 i (1-indexed) に対応する cpu_time を searchsorted で一括取得
        pt_indices = np.arange(1, N + 1)
        idx = np.searchsorted(ev_sorted, pt_indices, side='left')
        idx = np.clip(idx, 0, len(cpu_sorted) - 1)
        t_arr = cpu_sorted[idx]
    else:
        h_indices = np.minimum(np.arange(1, N + 1), len(hist) - 1)
        t_arr = np.array([
            float(hist[i]['cpu_time']) if hist[i].get('cpu_time') is not None else 0.0
            for i in h_indices
        ])
...
def _worker_trial_hv_curve(args):
    ...
    # times array を事前構築してループ内で再生成しない
    times = np.array([h['cpu_time'] for h in hist])
    pts_arr = np.asarray(pts, dtype=float)
    curve = []
    for t in t_grid_list:
        ...
        if kind == 'ga':
            ev = hist[idx - 1].get('evaluations')
            n_take = int(ev) if ev else int(idx * len(pts_arr) / len(hist))
        else:
            n_take = min(idx, len(pts_arr))
        n_take = max(0, min(n_take, len(pts_arr)))
        if n_take == 0:
            curve.append(0.0)
            continue
        sub = pts_arr[:n_take]
...
def _worker_union_hv_curve(args):
    ...
    # times arrays と pts arrays を事前構築
    times_list = [np.array([h['cpu_time'] for h in hist]) for hist in hist_list]
    pts_arrays = [np.asarray(pts, dtype=float) for pts in pts_list]
    curve = []
    for t in t_grid_list:
        pf_parts = []
        for hist, pts_arr, times in zip(hist_list, pts_arrays, times_list):
            if not hist or len(pts_arr) == 0:
                continue
            idx = int(np.searchsorted(times, t, side='right'))
            if idx <= 0:
                continue
...
def _compute_summary_for_md(method_data, methods, w_labels, baselines_by_method,
                              ...
                              region_hv_counts, union_pf_by_method, global_ref,
                              union_hv_by_n, init_ms, convergence_by_weight=None):
...
def generate_summary_md(all_summary, out_path, input_dir=''):
    ...
    lines = []
    lines.append('# 実験サマリ: core_comparison_v2')
    lines.append('')
    ...
    for prob_label in sorted(all_summary.keys()):
        ...
        # ---- 収束速度 ----
        conv_by_w = sd.get('convergence_by_weight', {})
        if conv_by_w:
            lines.append('### 収束速度 (代表重み別 HV 中央値, trial 間中央値)')
            lines.append('')
            lines.append('`@X%` = t_max の X% 時点。括弧内は最終値(100%t)に対する割合。')
            lines.append('')
            for wl, conv in sorted(conv_by_w.items()):
                if not conv:
                    continue
                sample_m = next(iter(conv))
                t_max = conv[sample_m]['t_max']
                fracs = _SUMMARY_CONV_FRACS
                hdr_cols = ''.join(
                    f' @{int(f*100)}%t ({f*t_max:.1f}s) |' for f in fracs[:-1]
                ) + f' @100%t ({t_max:.1f}s) |'
                lines.append(f'**{wl}**  (t_max={t_max:.1f}s)')
                lines.append('')
                lines.append(f'| 手法 |{hdr_cols}')
                lines.append('|------|' + ':---:|' * len(fracs))
                for m in methods:
                    cd = conv.get(m)
                    if cd is None:
                        lines.append(f'| {METHOD_LABELS.get(m, m)} |' + ' N/A |' * len(fracs))
                        continue
                    hv_final = cd['fracs'].get(1.00, 0.0)
                    row = f'| {METHOD_LABELS.get(m, m)} |'
                    for f in fracs:
                        hv = cd['fracs'].get(f, 0.0)
                        pct = f'{hv/hv_final*100:.0f}%' if hv_final > 0 else '—'
                        row += f' {hv:.2f} ({pct}) |'
                    lines.append(row)
                lines.append('')
    ...
    lines.append('---')
    lines.append('')
    lines.append('*generated by analyze_v2.py*')
    ...
    for wl in repr_w:
        ...
        for m in methods:
            ...
            for t_idx in sorted(by_trial.keys()):
                ...
                pts_list.append(get_uea_points(data, t_idx))
        ...
        for m, _, pts_list, _, bl in method_info:
            for pts in pts_list:
                if len(pts) == 0:
                    continue
                filtered = filter_baselines(np.array(pts), bl) if bl else np.array(pts)
                if len(filtered) > 0:
        ...
        convergence_by_weight[wl] = _compute_convergence_summary(
            method_info, w_ref, n_jobs=n_jobs)
...
def main():
    parser = argparse.ArgumentParser(description='core_comparison_v2 分析スクリプト')
    parser.add_argument(
        '--input-dir', required=True,
        help='run_v2.py の出力ディレクトリ')
    parser.add_argument(
