...
> **図の位置**：全幅図は Word のフロート表で、`![...]` 行の**直後の段落**があるページの上端に浮かぶ（`![...]` 行の位置は本文の順序ではなく、このアンカーを決めるためだけのもの）。図が乗り切らないページにアンカーが落ちると（別の図が既にあるページなど）、アンカー段落以降の本文が図と一緒に次ページへ押し出されて段が空く。空白が出たら `![...]` 行を前後の段落へ動かしてアンカーのページを変える。現在の配置：図 3 は「In ta21S, …」の段落の後ろ（アンカー＝表 2 キャプション。表 2 はキャプションと行を分割禁止にしてあるので塊で動く）、図 4 は §4.3 の結びの段落の後ろ（アンカー＝§5 見出し）。日本語版は本文の長さが違うので位置も異なる（図 3 は非対称の段落の後ろ、図 4 は全域 HV の段落の後ろ）。本文を削るたびにアンカーがずれるので、再生成後は PDF 化して段の空白を確認する。
...
The way Memetic-baseline fails to fill the region takes two forms (Figure 2). (a) In ta21S it has not a single non-dominated solution in the high-stability region, and needs $D$=16 to match the MS that ILS attains at $D$=2. (b) In ta21L it does enter the region but is inferior to ILS in MS, and the two are level only at the efficiency end. In either form, once the operators are mounted, the fronts of the two structures overlap in the high-stability region, and in five of the eight scenarios the non-dominated solutions of ILS+PR and Memetic+PR coincide exactly as point sets.
...
**Table 2.** Cross-scenario summary of the three metrics. Each cell gives "Friedman average rank / ARPD% (relative deviation from the best method of each scenario, averaged over the eight scenarios) / number of top-group scenarios (scenarios with no significant difference from the best method of that scenario, out of 8; two-sided Mann–Whitney U, uncorrected)"; lower is better for the first two and higher for the last. The Friedman test gives $p<$0.0001 for all three metrics, and Kendall's $W$ is 0.59, 0.81, and 0.63 in column order. Because the eight scenarios are not independent of one another (Section 4.1), the cross-scenario ranks are an exploratory summary of rank consistency.
...
**Overall HV.** The best level is reached by operator-equipped Memetic, which also occupies the top group in Table 2. The level of the ILS variants depends on the scenario: in four scenarios (la36S, ta21S, la40, la21) they are level with the best, but in the remaining four they fall 5–38% short. Between the two methods without operators the winner switches as well: ILS-baseline is significantly ahead in the four scenarios where the ILS variants are level with the best, and Memetic-baseline in the other four (in one of which the difference is not significant). The difference in the high-stability region in Section 4.2 is therefore not a reflection of ILS being superior overall. The way the operators act divides here in the same direction as in Section 4.2: they raise the overall HV of Memetic significantly in six of the eight scenarios (+2.8–11.7%; in the remaining two Memetic already exceeds ILS without operators), whereas for ILS the change stays within ±0.5% in every scenario, including the three in which the overall HV of ILS is 15–38% below the best. The minimum MS reached over the whole sweep shows the same picture: in the four scenarios where the ILS variants are level with the best, all methods except GA coincide, and in the other four the Memetic variants are lower (by 1.2–2.2% in three of them, $p<$0.001). The operators have no effect on this minimum.

**AOC.** The three ILS variants are level with the best, within 2% of the best in six scenarios (Table 2). In the remaining la36S and mt10, Memetic-baseline is best and the ILS variants fall 10–22% short. Because AOC weights the early phase heavily on the log-time scale, it strongly reflects the speed of ILS, which receives $S_p$ as its initial solution (Figure 4). The operators act asymmetrically here too, but in the opposite direction, as a cost to Memetic. PR traces the path all the way to $S_p$ and re-optimizes the intermediate solutions before returning, so each call is expensive, and in Memetic, where it fires on every individual, it lowers AOC significantly in five of the eight scenarios (by 7–26%). In ILS, where the operators fire only upon stagnation, the three variants stay within 3% of one another in every scenario, with no significant difference.
...
2. **H2 and the cross-structure comparison: the operators lift population-based search to the level of trajectory-based search and add little to trajectory-based search itself.** High-stability HV has a level at which several methods arrive and stop. Population-based search does not reach this level by itself, but with the operators mounted its high-stability HV rises in every trial and attains that level in seven of the eight scenarios. Trajectory-based search, which reaches the level by itself, gains little from the operators; they improve it only in the two scenarios where it falls short of the level on its own. Whether the operators help thus divides not along the difference between the structures but along whether the structure reaches this level by itself.
...
**Limitations.** (i) The results rest on n=10 (the main conclusions, which show complete separation, are robust; borderline cases are another matter). (ii) Stability is measured only by sequence deviation, and validity under start-time deviation is unverified (Section 3.1). (iii) The disruptions are limited to operation delays, which preserve the operation set. Under disruptions that change the operation set itself, such as rush-job insertion, $S_p$ ceases to be a complete reference solution and the very definition of stability (Eq. (1)) changes. (iv) Each scenario has a single $S_p$, and the effect of its quality on the results has not been examined. (v) Each search structure is represented by a single method (trajectory-based = ILS, population-based = Memetic, with PPX as the sole crossover).

**Future work.** Directions include re-verification with the conditions of (ii), (iii), and (v) above varied; integration of the scope-limiting approach with the operators (using them within the affected range); comparison with an $S_p$-biased crossover that restricts recombination toward $S_p$ (a direct test of the explanation behind H1); and re-verification with Pareto-native methods such as NSGA-II.
