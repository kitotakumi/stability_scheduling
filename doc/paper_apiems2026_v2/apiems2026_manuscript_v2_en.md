...
In predictive-reactive rescheduling the pre-disruption schedule $S_p$ is in hand before the search begins, and simply keeping its sequences is optimal for stability. Because large changes to $S_p$ are costly, the revised schedule actually adopted lies near it, so a method must be judged not only on the quality of the whole efficiency–stability trade-off but also on how well it fills the vicinity of $S_p$ (hereafter, the high-stability region). We therefore measure the hypervolume restricted to that region. With the local search held identical, trajectory-based search (ILS), which transforms $S_p$ in small steps, exceeded population-based search (Memetic), the mainstream choice, on this measure in every trial of all eight scenarios. We then equipped both structures with stability-inducing operators, derived from path relinking, that pull solutions back toward $S_p$; their effect was asymmetric. They raised the high-stability hypervolume of population-based search in every trial, to the level of trajectory-based search in seven of the eight scenarios, while leaving trajectory-based search, which fills this region by itself, almost unchanged. The best method changes with the criterion: population-based search with the operators for the quality of the whole trade-off, trajectory-based search for anytime performance.
...
In rescheduling, the original schedule $S_p$ from before the disruption is already in hand when the search begins. With the aim of securing stability by making use of it, this study examined the relationship between search structure and stability-inducing operators through a controlled comparison over 8 scenarios × 7 methods × n=10 trials, with the local search, the objective function, the supply of $S_p$, and the budget all aligned. The main findings are as follows.
...
The central contribution of this study is to establish an evaluation protocol that judges a rescheduling method by **the quality attained within the high-stability region**, and to show on that basis, in a controlled comparison with the local search aligned, that population-based search, the mainstream choice, does not fill this region fully and that supplying the move toward $S_p$ as an **operator** makes up the shortfall. The value of an operator is decided by whether the structure it is added to already fills this region by itself. A report that an operator is effective cannot be generalized without reference to the structure it was added to.
