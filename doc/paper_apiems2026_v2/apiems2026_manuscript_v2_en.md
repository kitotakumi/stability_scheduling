...
- **AOC** (anytime performance; López-Ibáñez and Stützle, 2014): the time average of the per-weight HV-versus-log-time curve.
...
**Statistics.** Method comparisons within each scenario are evaluated by the Mann–Whitney U test and the effect size Cliff's $\delta$ (Derrac et al., 2011), and all tests are two-sided. $|\delta|$=1.0 means complete separation: every trial of one method exceeds every trial of the other. The magnitude of a difference is reported as the ratio or relative difference of medians and as the relative deviation from the best method, ARPD% ($(\text{best}-x)/\text{best}\times100$, lower is better). In the cross-scenario summary (Table 2), each method is described by three quantities: its rank averaged over the eight scenarios, its mean ARPD%, and the number of scenarios in which it belongs to the top group, i.e., shows no significant difference from the best method of that scenario (uncorrected).

**Correspondence between hypotheses and tests.** H1 is tested by comparing the high-stability HV of ILS-baseline and Memetic-baseline, which share the local search, and is regarded as supported if ILS is significantly ahead in all eight scenarios. H2 is tested in the same form on the gain in high-stability HV of operator-equipped Memetic over Memetic-baseline, and the level the gain reaches is checked by comparison with ILS-baseline. For the corresponding comparison on the trajectory side (ILS-baseline versus operator-equipped ILS), we do not predict the direction of the effect in advance but contrast it with the population side as a difference in how the operators act. Multiple comparisons are handled by Holm correction, with the same comparison repeated across the eight scenarios forming one family. Neither hypothesis places a directional prediction on overall HV or AOC; these are reported as comparisons among methods.
...
**Table 2.** Cross-scenario summary of the three metrics (Section 3.4). Each cell gives "average rank / ARPD% / number of top-group scenarios (out of 8)"; lower is better for the first two and higher for the last. Because the eight scenarios are not independent of one another (Section 4.1), the cross-scenario ranks are an exploratory summary of rank consistency.
...
**AOC.** The three ILS variants are level with the best, lying within 2% of it in six scenarios (Table 2). In the remaining la36S and mt10, Memetic-baseline is best and the ILS variants fall 10–22% short. Because AOC weights the early phase heavily on the log-time scale, it strongly reflects the speed of ILS, which receives $S_p$ as its initial solution (Figure 4). The operators act asymmetrically here too, but in the opposite direction, as a cost to Memetic. PR traces the path all the way to $S_p$ and re-optimizes the intermediate solutions before returning, so each call is expensive, and in Memetic, where it fires per individual, it lowers AOC significantly in five of the eight scenarios (by 7–26%). In ILS, where the operators fire only upon stagnation, the three variants stay within 3% of one another in every scenario, with no significant difference.
