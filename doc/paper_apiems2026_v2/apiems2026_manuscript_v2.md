# [APIEMS 2026 投稿用・v2] Trajectory- and Population-Based Search with Path Relinking for Job-Shop Rescheduling under Sequence-Deviation Stability
...
**Title**: 処理順序の変更量を考慮したジョブショップ再スケジューリングにおける単点探索と多点探索の比較と path relinking の適用
（英: *Trajectory- and Population-Based Search with Path Relinking for Job-Shop Rescheduling under Sequence-Deviation Stability*）

**Abstract（≈380字 / 英200語未満）**

再スケジューリングでは、元計画 $S_p$ からの大幅な変更がコストを伴うため、修正計画には効率と、$S_p$ からの変更の小ささ（安定性）が同時に求められる。本研究は、高品質な初期解 $S_p$ が与えられるこの問題では、それを小刻みに変形していく単点探索（ILS）のほうが、既存研究で主流の多点探索（Memetic）よりも $S_p$ の近傍（以下、高安定領域）を良く埋めるという仮説を、局所探索を揃えた統制比較で検証した。この領域に限った HV（高安定 HV）は全 8 シナリオの全試行で単点探索が上回り、多点探索は 3 シナリオでほぼ届かず、残る 5 つでも 2.0〜4.5 倍の開きがあった。さらに、Path Relinking に基づき解を $S_p$ へ引き戻す安定性誘導演算子を両構造に載せると、効果は非対称に現れた。多点探索の高安定 HV は全試行で押し上がり、8 シナリオ中 7 つで単点探索の水準に達したが、自力でこの領域を埋めている単点探索はほとんど動かなかった。最良手法は評価軸で替わり、トレードオフ全体の品質では多点探索＋演算子、anytime 性能では単点探索が優れる。
...
本研究が対象とするジョブショップスケジューリング問題（JSSP）は NP 困難であり、実用規模では厳密解法でなくメタヒューリスティクスが用いられる [13]。これらは、1 つの解を少しずつ変形しながら進む**単点探索**と、複数の解を保持して互いに組み替えながら進む**多点探索**の二系統に大別される [30]。再スケジューリングを含む多目的な JSSP の解法は、GA を基軸とする多点探索が主流である（§2）。しかし、$S_p$ という高品質な初期解が与えられるこの問題では、**それを少しずつ変形していく単点探索のほうが、高安定領域を埋めるのに向いているのではないか**。これが本研究の出発点である。
...
HV はシナリオ横断比較のため各シナリオで $[0,1]^2$ にアフィン正規化し参照点 $(1.1,1.1)$ で算出する。$S_p$ の機械順序を保ったままの解（$D=0$。$S_{RSR}$ など）は全手法が探索せずに得る自明解なので、HV の算出から除く。全域 HV と高安定 HV は各試行で重み掃引の全点の解を合併して算出し、AOC は逆に**重みごとの探索を単位**として、壁時計の対数時間で平均した値を試行ごとに全重みで平均する（時間窓は各重みで全手法共通）。

**統計.** 各シナリオ内の手法比較は Mann–Whitney の U 検定と効果量 Cliff's $\delta$ で評価し、検定はすべて両側で行う。$|\delta|$=1.0 は一方の全 trial が他方の全 trial を上回る完全分離を意味する。差の大きさは中央値の倍率・相対差で示す。
...
Memetic-baseline の埋め損ね方は二つの型に分かれる（図 2）。(a) ta21S では高安定領域に非劣解を 1 点も持たず、ILS が $D$=2 で得る MS に並ぶには $D$=16 を要する。(b) ta21L では入りはするが MS で ILS に劣り、両者が並ぶのは効率端のみである。いずれの型でも演算子を載せると両構造のフロントは高安定領域で重なり、この領域内では 8 シナリオ中 5 つで ILS+PR と Memetic+PR の非劣解が点集合まで一致する。
...
![図3. 探索構造 × 安定性誘導演算子の交互作用図（高安定 HV、中央値と四分位範囲、n=10）。記号は演算子なしに対する両側 Mann–Whitney U 検定の有意水準で、ILS は線の上・Memetic は線の下に置く（\* $p<$0.05, \*\* $p<$0.01, \*\*\* $p<$0.001、多重比較補正なし）。](figures/fig_v2_interaction_en.png)
...
**PR か repair か.** ILS ではどちらを載せてもほぼ同じで、3 指標とも差は 2% 以内にとどまる。Memetic では評価軸で替わり、最終的な品質は有意差のあった 5 例（全域 HV 2/8、高安定 HV 3/8）がいずれも PR 優位、AOC では逆に repair が 6/8 で有意に上回る。
...
3. **実務上の含意：重視する軸で最良手法は替わる.** トレードオフ全体の品質（全域 HV）は Memetic＋安定性誘導演算子、高安定領域の品質（高安定 HV）は ILS と Memetic＋演算子の双方、anytime 性能（AOC）は ILS 系が優れる。ゆえに計算時間を許容してトレードオフ全体の品質を追求するなら Memetic＋演算子、変更の少ない再計画を早く出すなら ILS を選ぶ。Memetic に載せる演算子は、最終的な品質ではわずかに PR、anytime 性能では repair が上回る（§4.3）。
...
> ※ 番号は母艦 [research_document.md](../research_document.md) のものを暫定流用（本文中の [n] と対応）。最終版では出現順に 1 から振り直す。[28] は未引用のため削除。
...
6. **参考文献を初出順に振り直す**（現在 25 件だが番号は v1 から流用していて 2・7・10・19・20・21・28・31・32・33 が欠番。本文の引用とリストは一致しているので壊れてはいないが、投稿前に連番へ直す）。
