...
- **H2：演算子は Memetic の高安定 HV を全 8 シナリオの全試行で押し上げ、8 シナリオ中 7 つで ILS の水準に届かせる**。押し上げは全シナリオで完全分離（$p<$0.001, $|\delta|$=1.0）。演算子なしでは高安定領域にほぼ解を持てなかった 3 シナリオで解が現れ、残る 5 シナリオでは高安定 HV が 2.1〜4.7 倍になる。到達点は 7 シナリオで ILS-baseline の水準に届く：6 つで有意差がなく（うち 5 つは中央値の差 1% 以内）、ta21L では 4〜5% 有意に上回る。届かないのは la40 だけで、そこでも 5〜10% 下回るにとどまる（Holm 補正後 $p_{\text{adj}}<$0.015）。H2 はこの 1 シナリオを除いて支持された。押し上げが摂動一般の効果か、$S_p$ 方向に固有の効果かは、強度を揃えて方向だけをランダム化した対照実験を 2 シナリオ（mt10・la36S）で回して確かめた：ランダム方向は押し上げの 7 割前後しか再現せず、高安定 HV では演算子に完全分離で及ばず（$p<$0.001）、同水準に達するには 1.7〜2.4 倍の時間を要する。
...
**表 2.** 3 指標の横断要約。各セルは「Friedman 平均順位／ARPD%（各シナリオの最良手法からの相対偏差の 8 シナリオ平均）／首位群シナリオ数（そのシナリオの最良手法と有意差がないシナリオ数, /8。両側 Mann–Whitney U, 補正なし）」で、前 2 者は小さいほど、最後は多いほど良い。Friedman 検定は 3 指標とも $p<$0.0001、Kendall's $W$ は列の順に 0.59・0.81・0.63。8 シナリオは互いに独立でない（§4.1）ため、横断順位は順位一貫性の探索的要約である。
...
**全域 HV.** 最良の水準に達するのは演算子を載せた Memetic で、表 2 でも首位群を占める。ILS 系の水準はシナリオに依存し、4 シナリオ（la36S・ta21S・la40・la21）では最良に並ぶが、残る 4 つでは 5〜38% 下回る。演算子をもたない 2 手法どうしでも勝者は入れ替わり、ILS 系が最良に並ぶ 4 シナリオでは ILS-baseline、下回る 4 つでは Memetic-baseline が有意に上回る（うち 1 つは有意差なし）。ゆえに §4.2 の高安定領域の差は、ILS が全体として優れていることの反映ではない。演算子の効き方はここでも §4.2 と同じ向きに分かれる：Memetic の全域 HV を 8 シナリオ中 6 で有意に押し上げる（+2.8〜11.7%。残る 2 つは演算子なしで既に ILS を上回る）のに対し、ILS では全シナリオで ±0.5% 以内にとどまる（最良を 15〜38% 下回る 3 シナリオでも同じ）。掃引全体で到達した最小 MS も同じ構図で、ILS 系が並ぶ 4 シナリオでは GA を除く全手法が一致し、下回る 4 つでは Memetic 系が低い（3 つで 1.2〜2.2%, $p<$0.001）。演算子はこの最小 MS には効かない。
...
**AOC.** 最良の水準に並ぶのは ILS 系 3 手法で、6 シナリオで最良から 2% 以内にある（表 2）。残る la36S・mt10 では Memetic-baseline が最良で ILS 系は 10〜22% 下回る。AOC は対数時間で早期を重く見るため、$S_p$ を初期解として受け取る ILS の早さを強く反映する（図 4）。効き方はここでも非対称だが、向きは逆で Memetic のコストになる。PR は $S_p$ までの経路を辿り切って中間解を再最適化するため 1 回が重く、個体ごとに発動する Memetic では AOC を 8 シナリオ中 5 で有意に下げる（7〜26%）。停滞時にしか発動しない ILS では 3 手法の差が全シナリオで 3% 以内かつ有意でない。
...
**限界.** (i) n=10 に基づく（完全分離した主結論は頑健だが境界事例は別）。(ii) 安定性を順序偏差でのみ測っており、開始時刻偏差下での妥当性は未検証である（§3.1）。(iii) 外乱は作業集合を保つ作業遅延型に限る。特急ジョブの割込みのように作業集合そのものを変える外乱では、$S_p$ が完全な参照解でなくなり安定性（式(1)）の定義自体が変わる。(iv) 各シナリオの $S_p$ は 1 つであり、その品質が結果に与える影響は検証していない。(v) 各探索構造の代表は 1 手法ずつである（単点探索＝ILS、多点探索＝Memetic、交叉は PPX の 1 方式）。

**今後の課題.** 上記 (ii)(iii)(v) の条件を変えた再検証に加え、範囲限定型と演算子の統合（影響波及範囲内での併用）、組み替えを $S_p$ 寄りに制限した $S_p$ 偏向交叉との比較（H1 の説明の直接検証）、および Pareto-native 手法（NSGA-II 等）での再検証が挙げられる。
