...
予測リアクティブ再スケジューリングでは、外乱前の元計画 $S_p$ が探索開始前から手元にあり、その順序を保つだけで安定性は最適になる。変更にはコストが伴うため、実際に採用される修正計画は $S_p$ の近くにある。ゆえに手法は、トレードオフ全体の品質だけでなく、$S_p$ 近傍（以下、高安定領域）をどれだけ良く埋められるかでも評価すべきであり、本研究はこの領域に限った HV を測る。局所探索を揃えた統制比較の結果、$S_p$ を小刻みに変形する単点探索（ILS）は、主流である多点探索（Memetic）を全 8 シナリオの全試行で上回った。解を $S_p$ へ引き戻す安定性誘導演算子（Path Relinking に基づく）を両構造に搭載すると、効果は非対称に現れた。演算子は多点探索の高安定 HV を全試行で押し上げ、8 シナリオ中 7 つで単点探索の水準に到達させた一方、自力でこの領域を埋めている単点探索はほとんど変えなかった。最良手法は評価軸で替わり、トレードオフ全体の品質では多点探索＋演算子、anytime 性能では単点探索が優れる。
