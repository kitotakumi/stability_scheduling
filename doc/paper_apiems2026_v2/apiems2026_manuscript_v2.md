...
- **AOC**（anytime 性能 [26]）：重みごとの HV-対-対数時間曲線の時間平均。
...
**統計.** 各シナリオ内の手法比較は Mann–Whitney の U 検定と効果量 Cliff's $\delta$ で評価し [32]、検定はすべて両側で行う。$|\delta|$=1.0 は一方の全 trial が他方の全 trial を上回る完全分離を意味する。差の大きさは中央値の倍率・相対差と、最良手法からの相対偏差 ARPD%（$(\text{best}-x)/\text{best}\times100$, 低いほど良い）で示す。横断要約（表 2）では各手法を、8 シナリオでの平均順位・ARPD% の平均・首位群シナリオ数（そのシナリオの最良手法と有意差がないシナリオ数。補正なし）の 3 つで表す。
...
**表 2.** 3 指標の横断要約（定義は §3.4）。各セルは「平均順位／ARPD%／首位群シナリオ数（/8）」で、前 2 者は小さいほど、最後は多いほど良い。8 シナリオは互いに独立でない（§4.1）ため、横断順位は順位一貫性の探索的要約である。
