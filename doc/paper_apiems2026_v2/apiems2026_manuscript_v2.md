...
ILS [22] は、現在解に摂動を加え、そこから局所探索で解を改善し、得られた解を受理するかを判定する、という反復で 1 つの解を改善していく枠組みである。単点探索の中で ILS を採る理由は、深掘り（局所探索）と脱出（摂動）が明確に分離されている点にある。この分離ゆえに、**摂動強度——現在解から一度にどれだけ離れるか——を挿入手数として直接制御**でき、安定性誘導演算子（§3.3）も摂動として自然に組み込める。**局所探索**は ILS と Memetic で共有する JSSP 標準の N5 近傍 [1] で、候補をクリティカルブロック端の隣接 swap（makespan 改善見込みのある手）に絞り、採否は $F(\lambda)$ で評価する。**摂動**は insert 移動で、強度（挿入手数）を停滞度に応じ鋸歯状に巡回させる（VNS [10] 型）。**受理判定**は改善時受理（better acceptance）で、摂動後に得た局所最適が best の $F(\lambda)$ を厳密改善したときのみ現在解を置き換える。
...
HV はシナリオ横断比較のため各シナリオで $[0,1]^2$ にアフィン正規化し参照点 $(1.1,1.1)$ で算出する。$S_p$ の機械順序を保ったままの解（$D=0$。$S_{RSR}$ など）は全手法が探索せずに得る自明解なので、HV の算出から除く。全域 HV と高安定 HV は、各試行で重み掃引の全点の解を合併してから算出する。AOC は逆に**重みごとの探索を単位**として測る：各重み・各試行の HV($t$) を壁時計の対数時間で平均し（時間窓は各重みで全手法共通）、得た値を試行ごとに全重みで平均する。この 3 指標は互いに独立ではない：高安定 HV は全域 HV と同じ点集合を $D<P_\theta$ で切り出した部分であり、AOC は全域 HV に至る曲線を重みごとに測った対数時間平均で、各曲線の終点はその重みの最終 HV に一致する。
...
**仮説と検定の対応.** H1 は、局所探索を揃えた ILS-baseline と Memetic-baseline の高安定 HV の比較で検定し、全 8 シナリオで ILS が有意に上回れば支持されたとみなす。H2 は、Memetic-baseline に対する Memetic＋演算子の高安定 HV の伸びを同じ形で検定し、押し上げの到達点は ILS-baseline との比較（差が有意でないシナリオを同水準とみなす）で確かめる。単点探索側の対応する比較（ILS-baseline 対 ILS＋演算子）については効果の向きを事前に予測せず、多点探索側との効き方の違いとして §4.2 で対比する。多重比較は、各仮説について 8 シナリオを 1 族として Holm 補正で扱う（演算子効果は探索構造ごとに族を分ける）。全域 HV と AOC にはいずれの仮説も方向の予測を置かず、手法間の比較として報告する。
...
| 計算予算 | ILS 3000 反復／GA・Memetic 500 世代（早期終了なし・全 run を上限まで実行）。反復と世代は単位が異なるため族内（ILS 系どうし・多点探索系どうし）で公称値を揃え、手法横断の anytime 性能は壁時計上の AOC で比較する |
...
| 単点探索（ILS） | insert 摂動の強度（挿入手数）2〜5 手を鋸歯状に巡回 |
| 演算子（多点探索側） | 各個体に確率 0.3 で発動。PR は経路上位 3 中間解を N5 で再最適化し最良を返す。repair の深さは 1〜$d$ から一様 |
| 演算子（単点探索側） | 停滞時に摂動として発動（無改善 400 反復で初回、以降は 10 反復）。PR は経路上最良の 1 解を返す。repair の深さは 1→$d$ を鋸歯状に掃き、best 改善で 1 に戻す |
| 環境 | AMD Ryzen 5 7530U／Python 3.12（NumPy・DEAP・SciPy）。全乱数シード固定 |

**妥当性の確認.** 評価側の境界 $P_\theta$ は比較手法プールに相対的に定まるため、$\theta$=50 を中心に 25〜75 で振って確認した。以降の結論は $\theta$ の選択に依存しない：H1 の完全分離は $\theta$=33〜67 で保たれて両端でも向きは変わらず、多点探索側の演算子効果は全閾値で有意（$p<$0.001）である（動くのは僅差で密集する上位群の内部順位のみである）。計算予算は、壁時計が族間で異なるにもかかわらず（Memetic+PR は ILS の 4.8〜9.4 倍）、中央値 run が全手法とも予算の 7 割以内で最終 HV の 99% に達しており、全域 HV の差は計算時間の差によるものではない。手法パラメータは、表 1 のうち手法の挙動を定める 11 軸——提案演算子側だけでなくベース探索構造側も——を標準値で固定したうえで、2 シナリオで中心値の上下に振る一要因感度分析にかけ、感度の大きさ（全域 HV で +0.5〜2%）が手法間の差（+7〜54%）より一桁小さいことを確かめた。
...
以上を並べると、同じ演算子の効果は載せる探索構造に依存して非対称に現れる。ただし両者を分けているのは構造そのものというより、構造が高安定領域に残す埋め残しの有無である：演算子は届いていない領域を埋めることで効くため、上記の共通の到達水準まで自力で埋めた探索構造には足す余地が残らない。この対応は 16 セル（2 構造 × 8 シナリオ）で一貫している——余地の残る多点探索側の 8 セルはすべて伸び、水準に達した単点探索側の 6 セルは伸びず、余地の残る単点探索側の 2 セルだけが伸びる。構造依存はこの媒介を通じて現れており、どちらの構造が埋め残しを残すかは H1 が示している。同じ発動設計のまま余地の有無だけで効きが変わる以上、非対称は演算子の発動回数の違い（表 1：多点探索では各個体に確率 0.3、単点探索では停滞時のみ）では説明できない。また演算子の組み込み方（返す解の数・深さの引き方, §3.3）は構造ごとに異なるが、組み込み方の異なる PR と repair で同じ非対称が出ており、結果はその細部には依らない。
...
ILS が探索開始の時点で既に高い全域 HV を持つのは、$S_p$ を初期解として受け取り最初の局所探索がそれを改善するためである。ta21S では Memetic＋演算子が ILS と同じ水準（1.17）に達するが、到達は ILS の 2.5 秒に対し 5.6〜7.8 秒を要する。la36L では多点探索が 7〜10 秒で ILS を追い越し、ILS が 0.80 で頭打ちになるのに対し 0.94 まで伸びる。曲線の終点と立ち上がりの速さは別の手法を最良と指す。
...
1. **H1：単点探索（ILS）は高安定領域を Memetic-baseline より良く埋める.** 高安定 HV で、同一 N5 を揃えた多点探索を全 8 シナリオで完全分離で上回った（$p<$0.001, $|\delta|$=1.0。5 シナリオで 2〜4.5 倍・残る 3 シナリオでは多点探索が高安定領域にほぼ届かない）。全域 HV では両者は互角である。この差は、局所探索・目的関数・$S_p$ の供給・予算を揃えた条件下で残ったものである（各構造の代表は 1 手法ずつであり、一般化の射程は限界 (v)）。
