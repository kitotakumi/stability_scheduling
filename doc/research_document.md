...
$$D(S_p, S_q) = \sum_{i \in M} \sum_{j \in J} \omega_{i,j} \left| r_{i,j}^p - r_{i,j}^q \right|$$

$$\omega_{i,j} = \frac{1}{(r_{i,j}^q)^\beta}$$

$r_{i,j}^p$、$r_{i,j}^q$ はそれぞれ $S_p$、$S_q$ における機械 $i$ でのジョブ $j$ の処理順位であり、$\omega_{i,j}$ は早期投入ジョブの順序変動を重くみる重みである。
...
- **パラメータ**: `repair_trigger`（停滞判定反復数）、`repair_strength`（swap 回数）

### 3.6 Memetic への機構適用と仮説

PR と repair を ILS に加えるのみならず、**Memetic（GA + N5 局所探索）** にも組み込む。その仮説的根拠を以下に示す。

ILS+N5 との組み合わせでは PR の効果に構造的な制約がある。N5 局所探索は MS を最適化する過程でクリティカルパス外の順序も含めて間接的に固定し、解から「MS を壊さずに安定性を改善できる余地（冗長性）」を除去する傾向がある。PR が経路上の中間解から改善を拾うにはこの余地が必要であるため、ILS+N5 の解はすでに冗長性が少なく PR が改善の余地を見つけにくい。加えて ILS は単一解ベースであるため、探索中の解が $S_p$ 近傍に留まりがちで PR の経路自体が短くなる。
...
C-metric（la21）では `C(Memetic+repair, Memetic-LS) = 1.000`、`C(Memetic+PR, Memetic-LS) = 1.000` と Memetic-LS を完全支配する。一方、ILS に対する PR・repair の C-metric に有意な差はない。**PR・repair は ILS に対してほぼ無効または逆効果、Memetic に対しては劇的に有効（+90〜+117）** というパターンが両問題で一貫して観察される。
...
#### PR・repair の有効性がベースアルゴリズムに依存する理由

**ILS に対して PR・repair が効かない構造的理由**: ILS の N5 局所探索は MS を最適化する過程でクリティカルパス（CP）外の順序も間接的に固定し、解から「MS を壊さずに安定性を改善できる余地（冗長性）」を除去する。PR が経路を辿る際、1 歩目から MS が悪化してしまい、経路上に出発点より良い中間解が存在しない。repair のランダム swap も ILS 自身の摂動と機能的に重複し、付加価値が薄い。

**Memetic に対して PR・repair が劇的に効く理由**: Memetic の GA コンポーネントが生成する解には N5 局所探索ほど厳密な MS 最適化が施されていないため、「MS を壊さずに安定性を改善できる冗長な順序変更」が残存している。PR はこの冗長性を活用して安定性を改善できる。また集団が収束した状態では交叉の多様化効果が失われるが、PR・repair は個体レベルの脱出機構として機能し、交叉では代替できない多様化を提供する。

この観察を整理すると、**PR・repair の効果はベースアルゴリズムが生成する解の D 値（$S_p$ からの偏差）の大きさに依存する**。ILS は $S_p$ 近傍の解を生成し続けるため PR が移動できる余地が小さく効果が出ない。一方 GA は $S_p$ から遠い高 D 値の解を多数生成するため PR の移動余地が大きく安定性を大幅に改善できる。この関係は 4.4 節の領域別 HV（Memetic+PR が低安定性帯の覆域を改善）によって直接裏付けられる。
...
1. **PR・repair の有効性はベースアルゴリズムの構造に依存する**。ILS+N5 に対しては局所探索による冗長性除去のために効果が限定的だが、Memetic に対しては劇的な改善をもたらす。この非対称性は、摂動機構が利用できる解の「余地（冗長性）」の有無に帰着する。
