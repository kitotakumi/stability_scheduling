...
def para_h1(text, first=False):
    # 見出しだけが段末に取り残されないよう、次の段落と必ず同じ段に置く
    return para_style(text, 'ima004' if first else 'Jima005', '<w:keepNext/>')
...
def build(template_path, out_path, blocks, fig_dir=''):
    ...
    for blk in blocks:
        ...
        if kind == 'title':
            ...
            # テンプレの ima100References は keepNext 付きで、全件が 1 塊として
            # 次ページへ送られ段末が大きく空く。段をまたいで流せるよう解除する
            # （keepLines は残し、1 件が途中で割れるのは防ぐ）
