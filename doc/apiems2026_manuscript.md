...
**なぜ NSGA-II 等の Pareto native 手法でなく重みスカラー化か.** 本研究が基本動作を重み付き和に据えるのは次の 3 点による。(i) **a priori 選好との整合.** 効率と安定性のどちらをどれだけ優先するかは意思決定者が定める量であり、重み付きスカラー化はこの優先度を**重みとして陽に与える** a priori 型の多目的最適化である [8]。実際 Rangsaritratsamee ら [3] は効率と安定性の二基準目的関数を重み付き和で構成し、「**意思決定者が効率と安定性の妥協点を選べるようにする**」ことをその動機として明示しており、この定式化は再スケジューリング研究で標準的に用いられてきた。確定した重みのもとで修正解は一意に定まり、これが現場（MES 等 [27]）へ展開される。(ii) **統制比較の共通基盤.** 本研究の主眼はホスト構造×機構の統制比較（§2）であり、単一解の軌道ベース（ILS）と集団ベース（Memetic）に**同一のスカラー目的 $F(\lambda)$ を共有させる**ことで初めて同一機構を両構造へ無改変で載せ、構造差だけを切り分けられる。NSGA-II 等は集団に固有の非劣ソート・混雑度を前提とし単一解軌道には載らないため、この統制設計（H1 の対比）が成立しなくなる。(iii) **運用整合的な速度（アンタイム）評価.** 運用では選好（重み）を一つ定めて解を一意に確定して展開する以上、重要なのは「**その優先度のもとで**どれだけ速く良い解に到達するか」という選好ごとの速度である。重みスカラー化は重み別のアンタイム HV($t$) を直接与え、掃引平均した AOC（§3.4）で速度を手法横断に比較できる。一方 Pareto front を一括最適化する NSGA-II 等は、運用で実際に展開される「特定選好のもとでの解の立ち上がり」に対応するアンタイム軌跡を自然には与えない。単一重みでのスカラー値比較は結論が重み選択に依存するため、重み $\lambda$ を複数点で掃引し UEA [9] のもとで全非劣解を統合して Pareto 覆域で評価する。**限界**として、重み付き和は非凸（凹）な Pareto front の凹部に到達できない凸限界をもつ [8]。ただし本研究の目的は真の Pareto front の導出ではなく**同一スカラー法のもとでの手法の横並び比較**であり、全手法が同じ凸限界を受けるため構造差の議論には支障しない（Pareto native 手法での再検証は今後の課題）。
...
- **軌道（ILS）：ほぼ頭打ち**。多くの問題で baseline 時点で高安定域を自力充填済みで天井タイ。機構が有意に効くのは**最高リスケ率帯の la36L（73%）と ta21_high（82%）のみ**（$p$=0.016, 0.001。とくに ta21_high は改善幅 0.029→0.031 と小だが方向は一貫）。これは機構効果が「無い」のではなく、ILS が近傍を自力で埋めきるため伸びしろが乏しく、外乱が極端に大きく充填が追いつかない場合のみ僅かな上積み余地が残ることを示す。
