...
**閾値頑健性.** P50 は比較手法プールに相対的なため、P25〜P75 で掃引して頑健性を確認した：H1・H2 が依拠する高安定比較はいずれの閾値でも逆転しない（H2 の集団側は全閾値で $p$=0.001、H1 の完全分離は P33〜P67 で保たれ、両端では効果量が弱まるのみで向きは変わらない）。
...
**発散型と収束型.** ILS（$S_p$ 起点に外へ広がる）と Memetic+PR（散らばった集団を $S_p$ へ引き寄せる）は類似した最終 Pareto 品質に到達するが、ILS は早期から良い incumbent を持ち、Memetic+PRはウォームアップ後に立ち上がる。AOC はこの交差を対数時間で集約して早期性能を反映するため、ILS 系が明確に優位となる。
