...
予測リアクティブ再スケジューリングでは外乱前の高品質スケジュール $S_p$ が既に存在し、修正解には効率（メイクスパン）と $S_p$ からの変更量の小ささ（安定性）が同時に求められるため、広域探索品質に加え $S_p$ 近傍（高安定領域）の充填度が重要な性能軸となる。本研究は N5 局所探索を揃えた統制比較で、軌道ベース（ILS）が連続変形でこの近傍を自力充填する一方、交叉で組み替える集団ベース（Memetic）は充填が構造的に粗いことを示す（H1）。さらに、Path Relinking の guiding solution を外乱前スケジュール $S_p$ に固定した再スケ向けの適応と、その 1 ステップ版である repair kick を、解を $S_p$ 方向へ引き寄せる安定性誘導演算子として与える。安定性誘導を探索演算子として実装するため軌道・集団の双方へ無改変で移植でき、安定性を範囲限定で確保する従来手法とは別のレバーをなす。8 シナリオ・7 手法・n=10 の実験から、同一演算子の効果がホスト構造に依存して非対称に現れること——機構は集団ベースの高安定域充填を大きく補い、集団を高安定で軌道に追いつかせ総合品質で首位へ押し上げる一方、近傍を自力充填済みの軌道では頭打ちとなる（H2）——と、評価指標で首位手法が入れ替わる相補構造を定量化する。
...
本研究の目的は次の 3 点である。(1) 高品質初期解 $S_p$ が存在するという問題特性が単一解ベース・集団ベースの探索挙動に与える影響の比較分析、(2) 安定性を積極的に向上させる探索機構——既存の Path Relinking を guiding solution $S_p$ 固定で再スケ向けに適応した PR と、その 1 ステップ版 repair——の設計とベース手法構造との相互作用の分析、(3) 速度・Pareto 覆域・安定性帯別性能を統合した多角的評価方法論の構築。これらを貫く中心的な見立ては、**高品質初期解 $S_p$ が存在するこの設定では、安定性を探索演算子として与えることが有効な設計レバーとなるが、その効果の大きさも探索手法どうしの優劣も、手法がもつ探索構造に強く依存する**というものであり、以下で実験的に検証する。

---

## 2. 既存研究と位置づけ

**効率と安定性の同時考慮.** JSSP は NP 困難でありメタヒューリスティクスが主流である。再スケジューリングにおける効率と安定性の同時最適化は Wu ら [14] を先駆けとし、JSSP では Rangsaritratsamee ら [3]（ハイブリッド GA）、Zhang ら [4]（GA＋Tabu Search）、フローショップでは Katragjini ら [19] などが取り組んできた。これらの目的の扱いは効率と安定性を重み付き和で単一スカラーに束ねる枠組みが主流だが（[14][3]）、いずれも**安定性は目的関数の評価項として受動的に組み込まれる**にとどまり、探索演算子そのものは安定性方向を志向しない。また解法は GA を基軸とする**集団ベースが中心**である。
...
**残された課題.** (a) 再スケジューリングには「高品質初期解が既に存在する」特殊性があるが、これと探索アルゴリズムの適合性を JSSP で正面から分析した研究はない。(b) 安定性を積極的に確保する既存の道は二系統に大別できるが、いずれも本研究とは異なる。**範囲限定型**は安定性を探索空間の制限で保証し、収束効率と構造的な安定性を得る一方、制限の外側に存在しうる効率‒安定トレードオフ解を探索対象から外す。**目的関数併合型**（重み付き和・多目的）は全空間を探索するが、安定性は評価項として受動的に働くのみで探索演算子は安定性方向を志向せず、高安定領域を充填できるか否かはベース探索構造の素の能力に委ねられる（本研究の H1 が示すとおり集団ベースはこれを構造的に苦手とする）。全空間を探索しつつ**安定性方向への誘導を探索演算子として能動的に組み込む**設計と、その効果がホスト構造にどう依存するかの分析は、いずれの系統でも行われていない。(c) 既存研究の多くは効率と安定性を重み付き和でスカラー化し（単一ないし少数の固定重みで）、hypervolume 等の Pareto 覆域指標・収束速度（アンタイム）・安定性帯別といった複数軸の評価方法論を欠く。

**本研究の位置づけ.** 本研究は安定性を制御するレバーを「範囲（スコープ）」ではなく「**演算子**」に置く。$S_p$ を初期解・初期集団に与えること自体は [27] とも共通だが、PR・repair は探索の途中でも解を $S_p$ 方向へ能動的に引き寄せる。安定性を探索空間の制限ではなく**探索演算子として確保する**——これが従来の範囲限定型 [15][16][17][27] に対する本研究の核心的な設計上の主張である。加えてこの機構は**特定のホストに固有でなく、軌道ベース（ILS）と集団ベース（Memetic）の双方へ無改変で移植できる**ため、同一機構を異なる探索構造に載せて**ホストと機構の相互作用を統制下で切り分ける**ことを可能にする。したがって本研究の主眼は最先端手法との性能競争（horse-race）ではなく、この演算子としての機構設計と、それが可能にするホスト構造×機構の相互作用の解明（構造分析）の双方にある。範囲レバーと演算子レバーは独立に設定できる別の設計次元であり（両者の統合＝厳密な直交性の実証は今後の課題）、比較は探索構造の対（軌道／集団・機構の有無）に統制して行う [28]。なお [27]（範囲レバー・MS 単一目的）は目的の置き方・解の返し方が本研究と異なり、同一ハーネス上での公平な直接比較には別設計を要するため、本研究では統制対象とせず今後の課題とする。
...
**集団ベース（GA / Memetic-LS）.** H1 の統制対照となる集団ベースは、個体を operation-based 表現 [25] で符号化し、交叉は precedence preservative crossover（PPX）[11]、突然変異は逆位（inversion）、選択はエリート保存付きトーナメントとする。**Memetic-LS** はこの GA バックボーンに上記 N5 局所探索を各個体へ Lamarckian に適用した memetic 手法（GA と局所探索の統合 [29]）で、局所探索をもたない素の GA は参考ベースラインとする。ILS と N5 を共有するため、両者の差は局所探索の有無ではなく探索構造（単一軌道 vs 集団＋交叉）のみに帰する。集団ベースの交叉は 2 親の構造を切り貼りする破壊的操作で生成子が $S_p$ 近傍から飛びやすく、N5 を備えた Memetic-LS でも高安定領域の充填が構造的に粗くなる——これが H1 の根幹である。
...
- **初期解・GA 設定**: $S_p$ は GA-500 で生成した高品質 active スケジュール（染色体を GT 法 [23] でデコード, §3.1 の非縮退条件を満たす）。GA バックボーン（$cx_{pb}$=0.85, $mut_{pb}$=0.1, pop=50）は標準値域に固定し独立変数としない。
...
H1・H2 で確立した構造を、全 7 手法 × 8 問題 × 3 指標の**総合スコアボード**（下図）で俯瞰し、冒頭で予告した「指標で首位が替わる」相補構造を裏付ける。3 指標で緑（最良）の分布が入れ替わることが「王者が替わる」ことの視覚的証拠である。Friedman 平均順位は 3 指標とも手法間で明確に分離し、横断的な順位一貫性は中〜大である（Kendall's $W$=0.59／0.81／0.63, $p$<0.001。ただし §3.4 のとおり 8 シナリオの相関ゆえ探索的要約）。統合 HV は §4.2 で ILS と互角だった指標だが、その首位 Memetic+PR は leave-one-out に頑健で（どの 1 シナリオを除いても首位不変）、僅差が単一シナリオに駆動されたものでないことを確認した。
...
**【探索的な気づき】統合 HV の勝者と再スケジューリング率.** §4.2 で互角だった統合 HV（総合品質）の勝者は再スケジューリング率と対応し、~50% を境に低で ILS・中〜高で Memetic と二分する（同一 la36 ラダー 27/54/73% で ILS→Memetic→Memetic）。この対応を捉えるのは再スケジューリング率のみで、可動工程の絶対数や問題規模では捉えられない——率が効くのは固定骨格の割合を織り込むためと考えられる。機序は次の連鎖で理解できる。**(i) 効率端の位置（推測）**：固定骨格が大きいほど骨格がメイクスパンの大半を $S_p$ のまま規定して効率のよい解が $S_p$ 近傍に集まり、小さいほど遠方に良い効率解が現れうる。**(ii) 到達可否**：安定性項が $S_p$（$D$=0）を引力点とするため、効率端が近いうちは ILS の近傍充填で足りるが、遠のくと単一軌道の ILS は引力に縛られて届かず、交叉で $S_p$ から離れた個体を保つ Memetic だけが到達する（§4.2 の H1 と同型）。この意味で再スケジューリング率は、単一軌道が $S_p$ の引力圏を脱して遠方の良解へ到達する難しさの代理と見なせる。ただし全ては説明できず探索的観察に留める：ta21_high（82%）は二分の例外（≒タイ, $p$=0.053）で、相関には初期解 $S_p$ 品質の交絡や距離の測り方（順列偏差）への依存が残る。以上より、この対応は作問を支える難易度軸の探索的見立てに留める。本命の高安定 HV 優位・機構非対称は、この対応やその交絡には依存せず、全 8 シナリオの直接検定で独立に確立している。
