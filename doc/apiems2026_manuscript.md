...
**安定性を機構として担う系譜（範囲限定型）.** 一方、安定性を**再スケジュール範囲の限定**で構造的に保証する系譜がある。match-up scheduling [15]、AOR（影響波及作業のみ再スケ）[16]、染色体表現を区間に限定する Zakaria & Petrovic [17]、近年では Sun ら [27] が範囲を 4 階層に形式化し影響波及範囲限定の ERRE を提案、同一の解法を全範囲に適用した変種との対照実験で、性能源泉が解法そのものではなく範囲限定機構にあることを示した。これらに共通するのは安定性を**探索空間の制限**で保証する点である。
...
**なぜ NSGA-II 等の Pareto native 手法でなく重みスカラー化か.** 本研究が基本動作を重み付き和に据えるのは次の 3 点による。(i) **実務適合.** 現場のスケジューラは「効率を $\alpha$ 割・安定性を $\beta$ 割重視する」という単一の優先度ポリシーで運用され、重み付きスカラー化はこの運用形態に直接対応する [8]。効率と安定性を重み付き和で同時最適化する再スケジューリングは先行研究でも標準的枠組みであり [3]、Pareto 集合を陽に返す MOEA を再スケジューリングに正面から適用した例はほとんどない。(ii) **統制比較の共通基盤.** 本研究の主眼はホスト構造×機構の統制比較（§2）であり、単一解の軌道ベース（ILS）と集団ベース（Memetic）に**同一のスカラー目的 $F(\lambda)$ を共有させる**ことで初めて同一機構を両構造へ無改変で載せ、構造差だけを切り分けられる。NSGA-II 等は集団に固有の非劣ソート・混雑度を前提とし単一解軌道には載らないため、この統制設計（H1 の対比）が成立しなくなる。(iii) **アンタイム（速度）評価.** 本研究は「いつ止めても良い解が得られるか」を AOC で評価する（§3.4）。重み別にアンタイム HV($t$) を測れる重みスカラー化は、軌道・集団を共通の時間窓上で apples-to-apples に比較でき、速度という評価軸を自然に扱える。単一重みでのスカラー値比較は結論が重み選択に依存するため、重み $\lambda$ を複数点で掃引し UEA [9] のもとで全非劣解を統合して Pareto 覆域で評価する。**限界**として、重み付き和は非凸（凹）な Pareto front の凹部に到達できない凸限界をもつ [8]。ただし本研究の目的は真の Pareto front の導出ではなく**同一スカラー法のもとでの手法の横並び比較**であり、全手法が同じ凸限界を受けるため構造差の議論には支障しない（Pareto native 手法での再検証は今後の課題）。
