...
ベンチマークは mt10・la21・la36・la40・ta21。外乱は計 8 シナリオ＝la36S/la36M/la36L（27/54/73%）・ta21S/ta21L（32/82%）・mt10（72%）・la21・la40（32%）で、各シナリオに付した百分率は**リスケ率** $\rho=n_{res}/\text{ops}$——全作業のうち外乱の影響を受け再最適化対象 $\mathcal{O}_{opt}$ となった作業の割合——を表す。残る凍結部（外乱時刻までに確定済みで動かせない）は解を $S_p$ に縛るアンカーとして働くため、動かせる工程が少ない低リスケ率（small）では**どの修正解も $S_p$ から小さくしか変えられず**、スケジュールの大半を組み替えられる高リスケ率（large）では**$S_p$ から大きく異なる修正解まで到達しうる**。この到達範囲の広狭がベース手法の優劣とも関わるため（§4.5）、リスケ率を主たる統制軸に据える。とりわけ **la36 ラダー**（27/54/73%）と **ta21 対**（32/82%）は、同一インスタンス・同一 $S_p$ のままリスケ率のみを段階的に変え、手法差の $\rho$ 依存性を交絡なく切り分ける目的で構成した。
