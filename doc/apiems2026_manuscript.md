...
- **高安定 HV（本命）：ILS が全 8 問題で完全優越**（2〜4.5 倍, $p$=0.001, $|\delta|$=1.0＝完全分離。低リスケ率の 3 問題では Memetic-LS が高安定域に解を 1 つも到達できず ∞ 倍）。
- **AOC：6/8 で ILS 有意優位**（例外 la36S・mt10 は小規模問題で集団の早期被覆が効く）。壁時計差の影響は全手法共通の積分窓と ILS の早期飽和により排除している（§3.4・§4.1）。
